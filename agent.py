...
def heartbeat():
    try:
        api_post("/api/agent/heartbeat", {"cluster": CLUSTER_NAME, "instanceId": AGENT_INSTANCE_ID, "version": "0.1.0"})
    except Exception:
        pass


if __name__ == "__main__":
    print(f"Deploy Agent started cluster={CLUSTER_NAME} instance={AGENT_INSTANCE_ID} platform={PLATFORM_URL}", flush=True)
    last_heartbeat = 0
    pending_result = None
    while True:
        try:
            ...
            if pending_result:
                ...
                pending_result = None
                continue
            result = api_get("/api/agent/tasks", {"cluster": CLUSTER_NAME, "instanceId": AGENT_INSTANCE_ID})
